...
#endif
#pragma comment(lib,"Msimg32.lib")  //	Imm32.lib
#pragma comment(lib,"Imm32.lib")
using namespace Scintilla;

char m_LEF[20] = { 0 };//记录行号
HIGHEDITARR   Aem;
...
//画行号
void  SurfaceGDI::OnDrawLineNum(HDC dc, PMyCEDIT sp, DWORD x, DWORD y)
{
	if (!sp || !dc)
	{
		return;
	...
	::SetBkMode(dc, 0);
	::SetTextColor(dc, Aem.m_LineColor);
	::TextOut(dc, x, y, sp->m_str, sp->m_Strlen);
	return;
}
//画文本
void  SurfaceGDI::OnDrawText(HDC dc, PMyCEDIT sp, DWORD x, DWORD y)
{
	...
	while (star < stlen)
	{
		...
		if (ret)
		{
			...
			::SetTextColor(dc, Aem.m_HighlightColor);

			::TextOut(dc, x+t_ww.cx, y, sp->m_str + star, Aem.GetEditStr(aend)->m_Strlen);
			star += Aem.GetEditStr(aend)->m_Strlen;
			aend = 0;
			continue;
		}
		if (sp->m_str[star] == ' ')
		...
		::SetTextColor(dc, Aem.m_FontColor);
		if (sp->m_str[star] < 0)
		{
			::TextOut(dc, x + t_ww.cx, y, sp->m_str + star, 2);
			star += 2;
			continue;
		}
		//画，号
		...
		{
			::SetTextColor(dc, Aem.m_HighlightColor);
			::TextOut(dc, x + t_ww.cx, y, sp->m_str + star, 1);
			star += 1;
			aend = 0;
			continue;
		}
		...
		if (sp->m_str[star] >= '0' && sp->m_str[star] <= '9' || (p[star] >= 'A' && p[star] <= 'F') || (p[star] >= 'a' && p[star] <= 'f'))
		{
			if (star == 0)
			{
				if (star == stlen - 1)
				{
					::SetTextColor(dc, Aem.m_DigitalColor);
					::TextOut(dc, x + t_ww.cx, y, p + star, 1);
					star += 1;
					continue;
				}
				ret = FALSE;
				...
				if (ret)
				{
					ret = FALSE;
					::SetTextColor(dc, Aem.m_DigitalColor);
					::TextOut(dc, x + t_ww.cx, y, sp->m_str + star, aend - star + 1);
					star += aend - star + 1;
					continue;
				}
		...
		if (star < stlen - 1)
		{
			if (p[star] == '/' && p[star + 1] == '/')
			{
				::SetTextColor(dc,Aem.m_NoteColor);
				::TextOut(dc, x + t_ww.cx, y, p + star, stlen - star);
				star += stlen - star;
				break;
			}
		}
...
SurfaceGDI::SurfaceGDI() noexcept {
	AddEditStr(cppKeyWords);//#define SET_EDITLINECOLOR              0x6                    //行号颜色
